...
- 항목 17 : 특수 멤버 함수들의 자동 작성 조건을 숙지하라
    ...
    - 이동 연산들은 필요할 때에만 작성되며, 클래스의 비정적 자료 멤버들에 대해 “**멤버별 이동**”을 수행한다. 또한, **기반 클래스 부분을 이동**한다. 만약 이동 연산을 지원하지 않는 경우엔 복사 연산을 수행한다. (이동 시, std::move를 사용한다)
    ...
    - 가상 소멸자도 default로 만들 수 있다. 소멸자를 직접 만들면, 이동 연산들을 자동으로 작성하지 않으니 만약 필요하다면 default로 만들어주자. 그러면 이동 연산들도 자동으로 작성된다.
    - 그냥 가능한 자동 작성에 의존하지 말고 default로 명시하는 습관을 가지자. 나중에 특수 멤버 함수를 선언해서 이동 연산이 비활성화 되면서 의도치 않게 복사 연산이 사용되는 상황이 생기는 것을 방지할 수 있다.
