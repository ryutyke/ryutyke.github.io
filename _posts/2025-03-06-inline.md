...
근데 위 코드처럼 일반적인 클래스 멤버 함수면 클래스 밖에 함수 정의를 둘 경우, 그 헤더를 다른 곳에서 include 하면 정의가 하나 더 생기면서 단일 정의 규칙(One definition rule)을 위반하기 때문에 컴파일 에러가 뜬다. 그래서 inline을 붙여야 한다! (클래스 내부에 정의되어 있는 함수들은 자동으로 inline화 된다.)
...
(예: `foo<int>(5)`를 A.cpp와 B.cpp에서 모두 사용하면, 두 번역 단위에서 각각 `foo<int>`의 인스턴스가 생성됩니다.)

분명 이런 이유로 동일한 인스턴스들이 여러 개 생기지만!
**링킹 과정에서 링커가 동일한 기계어 코드를 가진 템플릿 인스턴스를 하나로 병합합니다!**
즉, 템플릿 인스턴스는 링커가 중복을 제거하기 때문에 inline이 없어도 괜찮다는 것이다.

(이건 이 다음 나올 의문을 해결하기 위해 GCC 코드 뜯어보다가 발견한 것이다.)
ilnine 명시했을 시, COMDAT을 사용하여 여러 번역 단위에서 중복 정의 가능하게 한다.
[https://github.com/gcc-mirror/gcc/blob/master/gcc/d/decl.cc](https://github.com/gcc-mirror/gcc/blob/master/gcc/d/decl.cc)
...
## 다음 의문 (inline를 명시하는 것이 인라인 최적화에 미치는 효과)
...
# GCC 코드 분석
Github에 코드가 있었다. 
[https://github.com/gcc-mirror/gcc/tree/master/gcc](https://github.com/gcc-mirror/gcc/tree/master/gcc)
...
[https://github.com/gcc-mirror/gcc/blob/master/gcc/ipa-inline.cc](https://github.com/gcc-mirror/gcc/blob/master/gcc/ipa-inline.cc)
...
[https://ko.wikipedia.org/wiki/%ED%94%BC%EB%B3%B4%EB%82%98%EC%B9%98_%ED%9E%99](https://ko.wikipedia.org/wiki/%ED%94%BC%EB%B3%B4%EB%82%98%EC%B9%98_%ED%9E%99)
...
[https://github.com/gcc-mirror/gcc/blob/master/gcc/tree-core.h](https://github.com/gcc-mirror/gcc/blob/master/gcc/tree-core.h) 
```cpp
unsigned declared_inline_flag : 1;
```

[https://github.com/gcc-mirror/gcc/blob/master/gcc/tree.h](https://github.com/gcc-mirror/gcc/blob/master/gcc/tree.h)
```cpp
...
1. inline 명시가 인라인 최적화에 영향을 주긴 한다.
2. 어떤 영향을 주는가?
...
[https://github.com/gcc-mirror/gcc/blob/master/gcc/opts.cc](https://github.com/gcc-mirror/gcc/blob/master/gcc/opts.cc)
