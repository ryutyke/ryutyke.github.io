...
- 블루프린트/C++ 상호작용
    - 다양한 플래그 값 및 Category 등의 메타데이터
    - UCLASS의 프로퍼티와 함수 목록 (바인딩)
- 프로퍼티 리플리케이트
    - 메모리 오프셋을 통해 값 변경 확인 (DOREPLIFETIME)  
    - 직렬화, 역직렬화 : 필드 유형, 크기 확인 및 메모리 오프셋에 직접 쓰기
...
CoreUObject 모듈 StartupModule에서 FCoreDelegates::OnInit에 void InitUObject()를 바인드합니다.
AppInit()에서 FCoreDelegates::OnInit.Broadcast()가 호출됩니다.
